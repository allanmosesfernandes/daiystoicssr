...
const Share = ({ data }: { data: QuoteData }) => {
    ...
    const addToBookmarks = () => {
        toast('Quote added to bookmarks', {
            action: {
            label: 'View',
            onClick: () => {
    ...
    const needToLogin = () => {
        toast('Login to access bookmarks', {
            action: {
                label: 'Login',
                onClick: () => {
                    auth.signinRedirect();
    ...
    return (
        <>
            <div className="dropdown dropdown-top font-xl">
                <div tabIndex={0} role="button" className="btn btn-neutral group m-1 text-2xl font-mf-consent min-w-[150px] py-4 relative">
                    <span className="transition-all group-hover:opacity-0 group-hover:scale-90">Share quote</span>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="absolute w-8 h-8 opacity-0 transition-all group-hover:opacity-100 group-hover:scale-100">
                        <path
                            fill="currentColor"
                            fillRule="evenodd"
                ...
                <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm font-2xl">
                    <li className="">
                        <a href="">
                            <FacebookShareButton url={shareURL} quote={tweetText}>
                                Facebook
                            </FacebookShareButton>
                        </a>
                    </li>
                    <li className="">
                        <a href="">
                            <LinkedinShareButton url={shareURL} title={tweetText}>
                                Linkedin
                            </LinkedinShareButton>
                        </a>
                    </li>
                    ...
                    <li>
                        <a>
                            <WhatsappShareButton url={shareURL} title={tweetText} separator="\n\n">
                                Whatsapp
                            </WhatsappShareButton>
                        </a>
                    </li>
            ...
            <button className="btn m-1 text-2xl font-mf-consent min-w-[150px] py-6" onClick={auth.isAuthenticated ? addToBookmarks : needToLogin}>
